  ...
  if($_GET['action']=='epass'){
      ...
      if($pass !== $spass){
          ...
          $spage = '<a href="javascript:history.go(-1)" class="am-btn am-btn-success am-btn-xs am-btn-block"><i class="am-icon-reply"></i> 返回修改</a>';
           ...
           $res = $DB->query("select * from aliyun_admin where name='$user' and pass='$jpass' limit 1");
           $res = $DB->fetch($res);
           if($jpass !== $res['pass']){
              $msg ='<div class="am-alert am-alert-warning am-text-center am-text-xl" data-am-alert>旧密码不正确</div>';
              $spage = '<a href="javascript:history.go(-1)" class="am-btn am-btn-success am-btn-xs am-btn-block"><i class="am-icon-reply"></i> 返回修改</a>';
            }else{
              $token = md5($user.$spass.md5('blogle.cn'));
              $res = $DB->query("update aliyun_admin set pass='$spass',token='$token' where name='$user'");
              if($res){
